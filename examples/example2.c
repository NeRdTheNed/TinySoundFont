...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < tsf_get_presetcount(g_TinySoundFont); i++)
	{
		...
		ma_mutex_lock(&g_Mutex);
		g_TinySoundFont->interpolateMode = TSF_INTERP_WATTE_4P;
		tsf_note_off(g_TinySoundFont, i, Notes[i % 7]);
		tsf_note_on(g_TinySoundFont, i, Notes[i % 7], 1.0f);
		ma_mutex_unlock(&g_Mutex);
		ma_sleep(1000);
		...
		ma_mutex_lock(&g_Mutex);
		g_TinySoundFont->interpolateMode = TSF_INTERP_HERMITE_4P;
		tsf_note_off(g_TinySoundFont, i, Notes[i % 7]);
		tsf_note_on(g_TinySoundFont, i, Notes[i % 7], 1.0f);
		ma_mutex_unlock(&g_Mutex);
		ma_sleep(1000);
		...
		ma_mutex_lock(&g_Mutex);
		g_TinySoundFont->interpolateMode = TSF_INTERP_LAGRANGE_4P;
		tsf_note_off(g_TinySoundFont, i, Notes[i % 7]);
		tsf_note_on(g_TinySoundFont, i, Notes[i % 7], 1.0f);
		ma_mutex_unlock(&g_Mutex);
		ma_sleep(1000);
		...
		ma_mutex_lock(&g_Mutex);
		g_TinySoundFont->interpolateMode = TSF_INTERP_BSPLINE_4P;
		tsf_note_off(g_TinySoundFont, i, Notes[i % 7]);
		tsf_note_on(g_TinySoundFont, i, Notes[i % 7], 1.0f);
		ma_mutex_unlock(&g_Mutex);
		ma_sleep(1000);
